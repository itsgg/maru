...
- **Plan:**
  ...
  - `args_prefix: []`
  - `seed: []` (see "Credential storage" below for current state)
- **Credential storage:** Codex documentation references three storage modes (`file`, `keyring`, `auto`), but as of Codex CLI 0.125.0 there is no documented `[auth] storage` key in `config.toml`, and `auth.json` appears to be the macOS default with no required directive (Phase 0 spike finding 0.5, [docs/spike-results.md](docs/spike-results.md)). Until the per-platform default is verified on Linux and Windows, no seed is written. Phase 1 live-smoke nightly CI must verify whether the keyring is the default on any platform; if it is, an issue tracks adding a per-platform seed at that time.
...
- **Validation:** profile dir is valid if it either does not exist (fresh) or contains `config.toml`.
